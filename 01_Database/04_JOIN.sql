/*
    ...
    (무작적 다 조회해 오는 것이 아니라 각 테이블간 연결고리(외래키)를 통해 데이터를 매칭시켜 조회해야한다.)

    JOIN은 크게 "오라클 전용구문"과 "ANSI 구문"
...
--2. DEPARTMENT와 LOCATION테이블을 참고하여 전체부서의 부서코드,부서명,지역코드,지역명 조회
...
/*
    ...
    단, 반드시 LEFT/RIGHT를 지정해야됩니다.(어떤 테이블이 기준이냐)
...
/*
    3.비등가 조인
    매칭시킬 컬럼에 대한 조건 작성 시 '='를 사용하지 않는 조인문
...
/*
    ...
    -하나의 테이블에서 서로 다른 두 개의 행을 비교해야하는 경우 사용
    -동일한 테이블을 조인하는 것이기 떄문에 별칭을 사용해서 구분해야한다.
*/
...
SELECT E.EMP_ID 사원번호,E.EMP_NAME 사원명,E.DEPT_CODE 사원부서코드,
        M.EMP_ID AS "사수 사번",M.EMP_NAME "사수의 사원명",M.DEPT_CODE "사수의 부서코드"
FROM EMPLOYEE E
...
/*
    내추럴조인(NATURAL JOIN)
    두 테이블 간에 공통된 컬럼 이름을 자동으로 기준으로 정해서 조인하는 방식 
    ON이나 USING 조건을 지정하지 않아도 두 테이블간 공통된 컬럼을 기반으로 조인\
    
    동일한 컬럼이 있을 경우에만 정상적인 조인이 가능하며 명시적이지 않고 조인조건을 정확하게 기술하기 힘들다.
*/
