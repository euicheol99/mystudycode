...
SELECT EMP_NAME 사원명, SALARY AS 급여, BONUS "사원의보너스", (SALARY*12) AS "연봉"
FROM EMPLOYEE;
...
SELECT EMP_NAME 사원명,SALARY 급여,SALARY*12 연봉,DEPT_CODE
FROM EMPLOYEE
...
/*
    ...
    EX) 비교할 대상 컬럼 LIKE '문자%' : 비교할 대상 컬럼값 중에서 해당문자로 시작하는 값들을 전부 가져온다.
        비교할 대상 컬럼 LIKE '%문자' : 비교할 대상 컬럼값 중에서 해당문자로 끝나는 값들을 전부 가져온다.  
        비교할 대상 컬럼 LIKE '%문자%' : 비교할 대상 컬럼값 중에서 해당문자가 포함된 값들을 전부 가져온다.
    
    2. '_': 한 글자를 대체검색할 때 사용
    EX) 비교할 대상 컬럼 LIKE '_문자' : 비교할 대상 컬럼값 문자 앞에 아무글자나 딱 한 글자가 있는 값을 조회
    EX) 비교할 대상 컬럼 LIKE '문자_' : 비교할 대상 컬럼값 문자 뒤에 아무글자나 딱 한 글자가 있는 값을 조회
    EX) 비교할 대상 컬럼 LIKE '_문자_' : 비교할 대상 컬럼값 문자 앞,뒤에 각각 아무글자나 딱 한 글자가 있는 값을 조회

     EX) 비교할 대상 컬럼 LIKE '___문자____' : 원하는 형태로 '_'개수를 정해서 추출할 수 있음
*/
...
/*
    ...
    <IN>
    WHERE절에 비교대상 컬럼값이 내가 제시한 목록 중에 일치하는 값이 있는지 검사하는 문법
