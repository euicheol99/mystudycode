...
--서브쿼리 
--예시1) 노옹철 사원과 같은 부서에 속한 사원들 전체 조회
...
--3047663보다 더 많은 급여를 받는 사원들의 사번, 이름 ,직급코드,급여조회
...
--4) 부서별 급여 합이 가장 큰 부서의 부서코드,급여 합
    --1) 부서별 급여 합이 가장 큰 부서의 급여 합(A라고 할 떄)
...
WHERE DEPT_CODE=(SELECT DEPT_CODE
                    FROM EMPLOYEE
                    WHERE EMP_NAME='전지연')
            AND NOT EMP_NAME='전지연'; --또는 AND EMP_NAME !='전지연';
...
--2) 대리직급임에도 과장 직급의 급여들 중에서 최소급여보다도 많이 받는 사원들의
--  사번,이름,직급,급여 조회
--2_1)과장 직급의 급여들
SELECT SALARY
FROM EMPLOYEE
...
--2_2)대리 직급이면서 위의 결과보다 급여가 하나라도 큰 사원의 사번,이름,직급,급여 조회
...
/*
    3.다중열 서브쿼리
    결과값은 한 행이지만 나열된 컬럼 수가 여러개일 경우(두 개 이상의 컬럼)
*/

--1) 하이유 사원과 같은 부서코드,같은 직급코드에 해당하는 서원들 조회
--사원명,부서코드,직급코드,입사일
...
WHERE (JOB_CODE,MANAGER_ID)=(SELECT JOB_CODE,MANAGER_ID
                                FROM EMPLOYEE
                                WHERE EMP_NAME = '박나라')
AND NOT EMP_NAME='박나라';

------------------------------------------------------------------------
/*
    4.다중행 다중열 서브쿼리
    서브쿼리의 조회 결과값이 여러 행,여러 열일 경우(주로 튜플 비교와 함께 사용)


*/

--1) 각 직급 별 최소급여를 받는 사원조회(사번,사원명,직급코드,급여)
...
--> 인라인뷰를 주로 사용하는 경우 --> TOP-N분석: 상위 몇 개만 조회
...
--별칭을 부여해야 한다.
SELECT ROWNUM,DEPT_CODE,SALARY_AVG
...
SELECT *
FROM(SELECT DEPT_CODE,SUM(SALARY),AVG(SALARY) AS SALARY _AVG,COUNT(*)
        FROM EMPLOYEE
        GROUP BY DEPT_CODE)
...
SELECT *
FROM(SELECT DEPT_CODE,SUM(SALARY),AVG(SALARY),COUNT(*)
        FROM EMPLOYEE
        GROUP BY DEPT_CODE
        HAVING AVG(SALARY)>2700000);
