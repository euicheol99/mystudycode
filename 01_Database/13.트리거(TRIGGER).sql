/*
    ...
    EX)
    회원탈퇴시 기존의 회원테이블에 데이터 DELETE한 후 
    곧바로 탈퇴한 회원들만 따로 보관하는 테이블에 자동으로 INSERT시켜야한다.
    ...
    *트리거의 종류
    ...
    -문장트리거 : 이벤트가 발생한 SQL문에 대해 딱 한 번만 트리거를 실행
    -행트리거 : 해당 SQL문 실행할 때마다 매번 트리거를 실행
                (FOR EACH ROW 옵션 기술)
