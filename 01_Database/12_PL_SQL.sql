...
-- 1_2) 레퍼런스 타입 변수선언및 초기화(어떤 테이블의 어떤 컬럼의 데이터를 참조해서 그 타입으로 지정)
...
-- 2) BEGIN 실행문
-- <조건문>
-- 2-1) IF 조건식 THEN 실행내용 END IF; (IF문 단독 사용일 경우)
...
--    ELSE DBMS_OUTPUT.PUT_LINE('BONUS : ' || BONUS);
    END IF;
...
EXCEPTION
    ...
    WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE('아무튼 예외 발생!');
END;
/
