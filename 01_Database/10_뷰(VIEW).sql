...
--CREATE OR REPLACE
--VIEW가 없을 떄는 생성, 이미 존재한다면 수정할 수 있다.
...
/*
    뷰 컬럼에 별칭 부여
    서비쿼리의 SELECT절에 함수식이나 산술연산식이 기술되어있다면 반드시 별칭을 부여 해야한다.
*/
...
/*
    *DML 명령어로 조작이 불가능한 경우
    ...
    2)뷰에 정의되어있지 않은 컬럼 중에 베이스테이블 상에 NOT JULL 제약조건이 지정되어있는경우
    3)산술연산식이나 함수식을 사용한 경우
    4)그룹함수나 GROUP BY절을 포함한 경우
    5)DISTINCT구문이 포함된 경우
    6)JOIN을 이용해서 여러테이블을 연관시켜놓은 경우

    대부분 뷰는 조회를 목적으로 사용한다. 그냥 뷰를 통한 DML은 안 쓰는게 좋다
*/
...
/*
    ...
    FORCE | NOFORCE
        ...
        ->NOFROCE: 서브쿼리에 기술된 테이블이 존재하는 테이블이어야만 한다.(기본값)
...
--200번 사원 급여를 200만원으로 변경(뷰의 WITH CHECK OPTION의 조건에 위배)
UPDATE VW_EMP
SET SALARY=2000000
WHERE EMP_ID=200;

-- WITH READ ONLY: 뷰에 대해 조회만 가능하도록 설정
CREATE OR REPLACE VIEW VW_EMP
