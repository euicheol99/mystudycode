...
<body>
	<h4><a href="el.do">바로가기</a></h4>
	<h4><a href="operation.do">02_EL의 연산자</a></h4>
	
	<h1>JSP Action Tag</h1>
	<%--
		*JSP를 이루는 구성
		1. 스크립팅 원소 : jsp 페이지 내에서 자바코드를 사용할 수 있는 기술
		ex) 스크립틀릿, 표현식 ...
		...
		3. 액션태그 : jsp 페이지에서 어떤 동작을 하도록 지시하는 태그
			...
			>>표준액션태그: jsp 페이지에서 바로 사용이 가능(별도의 연동이 필요없음)
					-> 모든 태그명 앞에 jsp: 접두어를 붙여서 사용 <jsp:태그명>
			>>커스텀액션태그: jsp 페이지에서 바로 사용 불가능(별도의 라이브러리 연동이 필요)
					-> 접두어에 jsp:이 아닌 모든 액션태그(가장 많이 사용하는 라이브러리 jstl)
	 --%>
	 <h3>*표준액션태그</h3>
	 <a href="views/2_standard_action_tag/01_include.jsp">01_jsp:include</a> <br>
	 <a href="views/2_standard_action_tag/02_forward.jsp">02_forward</a>
	 
	 <h1>JSTL이란?</h1>
	 <p>
	 	jsp에서 자주 사용하는 기능들을 태그형태로 제공하는 표준 라이브러리(커스텀 액션태그) <br>
	 	자바코드를 직접 사용하지 않고도 반복문, 조건문, 포멧처리... 쉽게 수행할 수 있음 <br>
	 	태그로 작성이 되어있기 때문에 코드 가독성이 높아짐.
	 </p>
	 
	 <h3>jstl라이브러리 추가</h3>
	 1)maven respository 사이트 접속 <br>
	 2)Jakarta Standard Tag Library Implementation와 Jakarta Standard Tag Library API
	 ...
	 <a href="views/3_custom_action_tag/01_core.jsp">3번</a>
	 
	 <h4>2. JSTL Formatting </h4>
