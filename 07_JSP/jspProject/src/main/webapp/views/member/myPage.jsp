...
<head>
	...
	<style>
        ...
        #update-delete-modal .modal-body form{
            display: flex;
            flex-direction: column;
        }
        #update-delete-modal .modal-body form h5{
            display: flex;
...
<body>
	...
	<div class="outer">
		...
		<form action="${pageContext.request.contextPath}/update.me" method="post" id="update-form">
			...
			<div align="center">
				...
				<button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#update-delete-modal">회원탈퇴</button>
			</div>
		</form>
	</div>
	      ...
	      <div class="modal-body">
            ...
            <script>
                document.getElementById('edit-pwd-btn').onclick = function(){
                    const pwd = document.querySelector("#update-pwd-modal input[name=userPwd]").value;
			        const pwdCheck = document.querySelector("#update-pwd-modal input[name=userPwdCheck]").value;
	...
	<!-- 
		...
		비밀번호 : 
		탈퇴하기 버튼 -> /delete.me
		=> /delete.me 받은 서버는 비밀번호가 맞는지 확인 후 status -> N으로 변경하고 로그아웃
	 -->
	 
	<!-- 회원탈퇴 팝업 -->
	<div class="modal" id="update-delete-modal">
	  <div class="modal-dialog">
	    <div class="modal-content">
	      ...
	      <!-- Modal body -->
	      <div class="modal-body">
			<form action="${pageContext.request.contextPath}/deletePwd.me" method="post">
				
                <table>
					<tr>
						<td colspan="2"><h5>정말 탈퇴하시겠습니까?</h5></td>
					</tr>
					<tr>
						<td><h5>비밀번호 :</h5></td>
						<td><input type="password" name="deletePwd" required></td>
					</tr>
				</table>
				<br>
				<button id="delete-pwd-btn" type="submit" class="btn btn-sm btn-primary">회원 탈퇴</button>
			</form>
            <script>
                document.getElementById('edit-pwd-btn').onclick = function(){
                    const pwd = document.querySelector("#update-pwd-modal input[name=userPwd]").value;
