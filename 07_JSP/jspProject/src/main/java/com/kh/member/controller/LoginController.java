...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(loginUser == null) { 
			...
			//포워딩 방식  -> 해당 url이 변경되지않는다.
			// -> 우리는 localhost:8002/jp/라는 기존의 메인 url이 있을 때
			// 포워딩 방식으로 응답시 url localhost:8002/jsp/login.me가 나타난다.
			// 실제화면은 성공시 메인화면 (localhost:8002/jp/), 실패시 errorPage.jsp가 나타나야한다.
			// 실패시 -> 응답화면으로 errorPage.jsp 나타나는게 맞으니 포워딩
			request.getRequestDispatcher("views/common/errorPage.jsp").forward(request, response);
		} else { 
			//로그인 성공
			//로그인된 회원정보를 session 영역에 담기(왜? 모든 페이지에서 사용하기 위해서)
			HttpSession session = request.getSession();
			session.setAttribute("loginUser",loginUser);
			session.setAttribute("alertMsg", "로그인성공");
			
			//url 재요청 방식
			//기존에 해당 페이지를 응답하는 url이 존재했다면 사용이 가능
			//응답페이지 -> index.jsp 페이지(jsp url 재요청)
